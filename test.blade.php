            ...
            <div class="projects-hero-content">
                ...
                @auth
                    @if(Auth::user()->role === 'seller')
                        <div class="seller-action-header" style="margin-top: 1.5rem;">
                            <a href="{{ route('seller.projects.create') }}" class="btn-pcu" style="background-color: #28a745; border-color: #28a745;">
...
@push('scripts')
<script>
    // 🔥 PENINGKATAN 3: Ambil data context user login secara detail ke JS global
    const currentUserId = {{ Auth::check() ? Auth::user()->id : 'null' }};
    const currentUserRole = "{{ Auth::check() ? Auth::user()->role : 'guest' }}";

    // Lempar data proyek murni dari database, lalu biarkan logic penentuan tombol dikelola di projects.js
    const projectsData = @json($projects).map(p => ({
        id: p.id,
        name: p.name,
        company: p.company_name,
        category: p.category,
        subcategory: p.methodology || 'N/A',
        duration: p.duration_months + " Bulan",
        description: p.description || '',
        price: p.price_per_ton,
        capacity: p.stock_available,
        sellerId: p.seller_id, // 🔥 Dilempar agar JS bisa membandingkan kepemilikan proyek kompetitor
        image: p.image ? `/images/${p.image}` : '/images/placeholder-project.jpg',
    }));
</script>
